class DeposerSujetJob < ApplicationJob
    ...
    def perform(sujet, intervenant_id = 0)
        ...
        mail_log = MailLog.create(user_id: intervenant_id, message_id: mailer_response.message_id, to: (sujet.cour.formation.courriel.present? ? sujet.cour.formation.courriel : sujet.cour.formation.user&.email), subject: "Sujet d'Examen déposé", title: title)
