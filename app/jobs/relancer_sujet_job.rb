class RelancerSujetJob < ApplicationJob
    ...
    def perform(sujet, user_id = 0)
        ...
        mailer_response = IntervenantMailer.relance_sujet(sujet, title).deliver_now
        mail_log = MailLog.create(user_id: user_id, message_id: mailer_response.message_id, to: sujet.cour.intervenant_binome.email, subject: "Relance Sujet Examen", title: title)
